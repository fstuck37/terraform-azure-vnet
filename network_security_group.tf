resource "azurerm_network_security_group" "security_groups" {
  for_each = { for k, v in var.subnets: k => v if k != var.gatewaysubnet_subnet_name}
...
resource "azurerm_network_security_rule" "lb-in-4095" {
  for_each = { for k, v in var.subnets: k => v if k != var.gatewaysubnet_subnet_name && k != var.public_subnet_name && var.default_deny_all}
